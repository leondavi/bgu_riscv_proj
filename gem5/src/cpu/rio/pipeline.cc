...
Pipeline::Pipeline(RioCPU &cpu_, RioCPUParams &params) :
		...
		activityRecorder(cpu.name()+".activity", Num_StageId , 1, 0) // TODO - fix the const 3
{
...
//    eToF1.evaluate();
    ...
    if (allow_idling) {
        ...
        if (!activityRecorder.active()) {
            std::cout<<"Suspending as the processor is idle\n";
            stop();
        }
        ...
        activityRecorder.deactivateStage(Pipeline::FetchStageId);
